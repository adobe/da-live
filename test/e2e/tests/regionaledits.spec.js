...
test('Regional Edit Document', async ({ page, context }, workerInfo) => {
  ...
  await newPage.locator('div.diff-color-overlay.diff-upstream').hover();
  await newPage.locator('da-diff-deleted').getByText('Delete', { exact: true }).click();
  await expect(newPage.getByText('Deleted H1 Here', { exact: true })).not.toBeVisible();

  await newPage.locator('div.diff-color-overlay.diff-local').hover();
  await newPage.locator('da-diff-added').getByText('Keep', { exact: true }).click();
  await expect(newPage.getByText('Added H1 Here', { exact: true })).toBeVisible();
  await expect(newPage.locator('div.diff-color-overlay.diff-local')).not.toBeVisible();
