...
describe('reaction-utils', () => {
  ...
  describe('toggleReaction', () => {
    ...
    it('adds reaction when user has not yet reacted', () => {
      ...
      const result = toggleReaction({ comment, emoji: '👍', user });
      expect(result.reactions['👍']).to.deep.equal([{ userId: 'u1', name: 'Alice' }]);
    });

    it('removes reaction when user has already reacted', () => {
      const comment = { reactions: { '👍': [{ userId: 'u1', name: 'Alice' }] } };
      const result = toggleReaction({ comment, emoji: '👍', user });
      expect(result.reactions['👍']).to.be.undefined;
    });

    it('removes the emoji key entirely when the last reactor removes their reaction', () => {
      const comment = { reactions: { '👍': [{ userId: 'u1', name: 'Alice' }] } };
      const result = toggleReaction({ comment, emoji: '👍', user });
      expect(Object.keys(result.reactions)).to.not.include('👍');
    });

    it('preserves other users reactions when toggling off', () => {
      const comment = { reactions: { '👍': [{ userId: 'u1', name: 'Alice' }, { userId: 'u2', name: 'Bob' }] } };
      const result = toggleReaction({ comment, emoji: '👍', user });
      expect(result.reactions['👍']).to.deep.equal([{ userId: 'u2', name: 'Bob' }]);
    });

    it('does not mutate the original comment', () => {
      const comment = { reactions: {} };
      toggleReaction({ comment, emoji: '👍', user });
      expect(comment.reactions['👍']).to.be.undefined;
    });

    it('returns a new object (immutable)', () => {
      const comment = { reactions: {} };
      const result = toggleReaction({ comment, emoji: '👍', user });
      expect(result).to.not.equal(comment);
    });
  });
