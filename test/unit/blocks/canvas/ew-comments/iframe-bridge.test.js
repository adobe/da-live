...
describe('iframe-bridge', () => {
  ...
  it('commentMarkers carries author color, initials and name', async () => {
    ...
    const authored = { ...comment, author: { name: 'Ada Lovelace', email: 'ada@x.com' } };
    const colorSet = { bg: '#123456', text: '#ffffff', strong: '#0000ff' };
    const controller = {
      selectedThreadId: 't1',
      getAttachedThreadIds: () => new Set(['t1']),
      getComment: () => authored,
      authorColorSet: () => colorSet,
    };
    const markers = commentMarkers(editor.view, controller);
    // Marker color comes from the controller's doc-derived slot, not the author record.
    expect(markers[0].color).to.equal('#123456');
    expect(markers[0].textColor).to.equal('#ffffff');
    expect(markers[0].highlightColor).to.equal('#0000ff');
  ...
  it('authorPresentation uses the provided colorSet, ignoring any stored author.color', () => {
    const present = authorPresentation(
      { name: 'Ada', email: 'ada@x.com', color: '#123456' },
      { bg: '#abcdef', text: '#111111', strong: '#222222' },
    );
    expect(present.color).to.equal('#abcdef');
    expect(present.textColor).to.equal('#111111');
    expect(present.highlightColor).to.equal('#222222');
  });
  ...
  it('postCommentMarkers posts markers and selectedThreadId', () => {
    ...
    expect(sent[0]).to.deep.equal({
      ...
      payload: { markers, selectedThreadId: 't1' },
    });
  });

  it('postScrollToComment posts scroll-to-pos (payload) for the selected thread', async () => {
    const { editor, controller } = await setup();
    const sent = [];
    postScrollToComment({ postMessage: (m) => sent.push(m) }, editor.view, controller);
    expect(sent[0].type).to.equal('scroll-to-pos');
    expect(sent[0].payload.proseIndex).to.be.a('number');
    destroyEditor(editor);
  });
});
