...
describe('comments helpers/controller', () => {
  ...
  it('heals a structurally-recovered anchor (moved page) by re-anchoring to the live ydoc', async () => {
    // Editor A = original page: encode the anchor against its ydoc.
    const edA = await createTestEditor();
    edA.view.dispatch(edA.view.state.tr.insertText('hello world'));
    const { encodeAnchor, resolveAnchor } = await import('../../../../../blocks/canvas/comments/helpers/anchor.js');
    const original = encodeAnchor({
      selectionData: { from: 1, to: 6, anchorType: 'text', anchorText: 'hello' },
      state: edA.view.state,
    });

    // Controller bound to editor B = the moved page: fresh ydoc, identical content.
    const { controller: c, editor: edB, store: m } = await createControllerWithPlugin();
    edB.view.dispatch(edB.view.state.tr.insertText('hello world'));
    m.set('t1', {
      id: 't1', ...original, author: { id: 'u' }, body: '', createdAt: 0, resolved: false,
    });

    // Precondition: relpos is dead in B, so it only resolves structurally.
    expect(resolveAnchor({ anchor: m.get('t1'), state: edB.view.state }).source).to.equal('structural');

    const attached = c.getAttachedThreadIds();
    expect(attached.has('t1')).to.be.true;

    // Healed: the stored anchor now resolves via a live relpos in B's ydoc,
    // so subsequent edits to the block track instead of detaching.
    expect(resolveAnchor({ anchor: m.get('t1'), state: edB.view.state }).source).to.equal('relpos');
