...
export default class DaVersionsBase extends LitElement {
  ...
  get _effectivePath() {
    return this.path || this._hashPath;
  }

  async getVersions() {
    const { _effectivePath: path } = this;
    if (!path) return;
    this._loading = true;
    this._versions = null;
  ...
  async handleNewSubmit(e) {
    ...
    const res = await daFetch(`${DA_ORIGIN}/versionsource${this._effectivePath}`, opts);
  ...
  update(changedProps) {
    ...
    if ((changedProps.has('path') || changedProps.has('_hashPath')) && this.open !== false) {
      if (this._effectivePath) this.getVersions();
    }
    super.update();
  }
