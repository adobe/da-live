...
export const COMMENT_ADD = 'da-comment-add';
export const COMMENT_EDIT = 'da-comment-edit';
export const COMMENT_DELETE = 'da-comment-delete';
export const COMMENT_CLOSED = 'da-comment-closed';
...
class DaCommentPanel extends LitElement {
  ...
  close() {
    this.dispatchEvent(new Event(COMMENT_CLOSED));
    this.remove();
  }
  ...
  handleAdd() {
    ...
    if (!text) return;
    this.dispatchEvent(new CustomEvent(COMMENT_ADD, { detail: { text }, bubbles: true }));
    this._newText = '';
  }
  ...
  saveEdit() {
    ...
    if (text != null && this._editingId) {
      const detail = { id: this._editingId, text };
      this.dispatchEvent(new CustomEvent(COMMENT_EDIT, { detail, bubbles: true }));
    }
    this._editingId = null;
    this._editText = '';
  }
  ...
  handleDelete(comment) {
    const detail = { id: comment.id };
    this.dispatchEvent(new CustomEvent(COMMENT_DELETE, { detail, bubbles: true }));
  }
    ...
    ? html`
          <div class="comment-edit-row">
            <input
              ...
              @keydown=${this.handleEditInputKeyDown}
            />
    ...
    : html`
        ...
        <div class="comment-add">
          <input
            ...
            @keydown=${this.handleNewCommentKeyDown}
          />
