...
export default class DaPreview extends LitElement {
  ...
  handleView({ key }) {
    ...
    if (!key) {
      const opts = { detail: false, bubbles: true, composed: true };
  ...
  setBody() {
    ...
    if (this.initialized) this.sendPreview();
  }
  ...
  get env() {
    return window.location.hostname.includes('local') ? 'local' : 'one';
  }
  ...
  get _source() {
    ...
    return `${src}?martech=off&dapreview=${this.env}`;
  }
