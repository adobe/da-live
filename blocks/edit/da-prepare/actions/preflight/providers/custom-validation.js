...
export function buildCustomValidationCategory({ results, timedOut, hasRunner }) {
...
export function runCustomValidationProvider(details, { signal } = {}) {
  return new Promise((resolve) => {
    const unsubscribe = canvasBus.validationResultState.subscribe((detail) => {
      unsubscribe();
      const { items, timedOut, hasRunner } = detail;
      resolve(buildCustomValidationCategory({ results: items, timedOut, hasRunner }));
    });
...
registerPreflightProvider(runCustomValidationProvider);
