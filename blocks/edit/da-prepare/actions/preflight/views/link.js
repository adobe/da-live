...
import { etcFetch, getSidekickConfig } from '../../../../../shared/utils.js';
...
class PreflightLink extends LitElement {
  ...
  async normalizeHref(supplied) {
    ...
    const aemOrigin = `https://main--${site}--${org}.aem.page`;
  ...
  async checkLink(url) {
    try {
      ...
      const resp = await etcFetch(noCacheUrl, 'cors', { method: 'HEAD', redirect: 'manual' });
  ...
  async getLinkDetails() {
    ...
    this._name = this.text || this._parts.at(-1);
