...
const topLevelAttrs = {
  ...
  daDiffDeleted: { default: null, validate: 'string|null' },
};
...
const getTopLevelToDomAttrs = (node) => {
  ...
  if (node.attrs.daDiffDeleted === '') {
    attrs['da-diff-deleted'] = '';
    attrs['data-mdast'] = 'ignore';
  }
  return attrs;
};
...
const getTopLevelParseAttrs = (dom) => ({
  ...
  daDiffDeleted: dom.getAttribute('da-diff-deleted') ?? null,
});
...
const baseNodes = {
  ...
  diff_deleted: {
    ...
    toDOM: () => ['da-loc-deleted', { 'data-mdast': 'ignore', contenteditable: false }, 0],
  },
};
