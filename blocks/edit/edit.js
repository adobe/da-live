...
async function getDoc(details) {
  const { source } = await getNx2Api();
  return source.get(details.fullpath);
}

async function createDoc(details) {
  const { source } = await getNx2Api();
  const data = new Blob([EMPTY_DOC], { type: 'text/html' });
  return source.put(details.fullpath, { body: data });
}
...
async function setUI(el) {
  ...
  const docPromise = getDoc(details);
  ...
  if (resp.status === 404) {
    ...
    const createResp = await createDoc(details);
